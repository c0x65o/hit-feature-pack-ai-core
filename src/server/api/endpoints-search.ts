...
function scoreEndpoint(q: string, ep: Endpoint): number {
  const hay = normalize(
    [ep.pathTemplate, ep.methods.join(' '), ep.summary ?? '', ep.methodDocs ? Object.values(ep.methodDocs).join(' ') : ''].join(
      ' '
    )
  );
  if (!hay) return 0;
...
export async function GET(request: NextRequest) {
  ...
  const endpoints: Endpoint[] = Array.isArray((data as any)?.endpoints) ? (data as any).endpoints : [];

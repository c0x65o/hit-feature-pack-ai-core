    ...
    for (const b of blocks) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            for (let j = i + 1; j < Math.min(lines.length, i + 4); j++) {
                ...
                if (/^(GET|POST|PUT|PATCH|DELETE)\s+\/api\//i.test(candidate))
                    break;
...
export function discoverAppApiEndpoints(projectRoot) {
    const now = Date.now();
